...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 13 10:34:00 2021
...
class Convolution3D(nn.Module):
    def __init__(self):
        ...
        self.frame_conv = ConvBlocks3D()
        self.motion_conv = ConvBlocks3D() 

        self.fc0_frames = nn.Linear(in_features =10488, out_features = 128, bias=True)
        self.fc0_motion = nn.Linear(in_features =10488, out_features = 128, bias=True)
        self.drop = nn.Dropout(0.25)
        self.elu = nn.ELU()
        self.encoder_layer = TransformerMotionEncoderLayer(d_model=128, nhead=4)
        self.transformer_encoder = TransformerMotionEncoder(self.encoder_layer, num_layers=1, norm=nn.LayerNorm(128))
    ...
    def forward(self, frames, optical_flow):
        ...
        optical_flow = self.motion_conv(optical_flow)
        #print(image.shape)
        frames = F.relu(self.fc0_frames(frames))#self.LSTM1(image)
        optical_flow = F.relu(self.fc0_motion(optical_flow))
        #image = image[0]
        #print(image.shape)
        #image = torch.tanh(image)
        
        # LSTM 16
        image = torch.tanh(self.transformer_encoder((frames, optical_flow)))

        # FC 512
        #print(image.shape)
        image = image.permute(1,0,2)
        #print(image.shape)
        image = self.elu(self.fc1(image))
        #print(image.shape)
        image = self.drop(image)
        # FC 128
        image = self.elu(self.fc2(image))
