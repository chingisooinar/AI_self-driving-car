...
class Convolution3D(nn.Module):
    def __init__(self):
        ...
        self.fc0 = nn.Linear(in_features =10488, out_features = 64, bias=True)
        ...
        self.fc1 = nn.Linear(in_features =64, out_features = 512, bias=True)
    ...
    def forward(self, Input):
        ...
        #print(image.shape)
        image = F.relu(self.fc0(image))#self.LSTM1(image)
        #image = image[0]
        #print(image.shape)
        #image = torch.tanh(image)
        
        # LSTM 16
        image = torch.tanh(self.transformer_encoder(image))
