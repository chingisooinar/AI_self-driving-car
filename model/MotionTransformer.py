...
class MotionTransformer(nn.Module):
    def __init__(self, seq_len):
        ...
        self.position_embedder = nn.Linear(in_features =1000, out_features = 512, bias=True)
        self.motion_embedder = nn.Linear(in_features =1000, out_features = 512, bias=True)
        
        self.encoder_layer = TransformerMotionEncoderLayer(d_model=512, nhead=4, dropout=0.1)
        ...
        self.reduce_combined = nn.Linear(in_features =512, out_features = 64, bias=True)
    ...
    def forward(self, frames, optical):
       ...
       # print(x.shape)
        ...
        frames = frames.reshape(-1,  self.seq_len, 512).permute(1, 0, 2)


        optical = F.relu(self.motion_embedder(self.motion_encoder(optical)))
        optical = optical.reshape(-1,  self.seq_len, 512).permute(1, 0, 2)
        ...
        fused_embedding = fused_embedding.reshape(-1, 512)
